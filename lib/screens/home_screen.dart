...
import 'package:flutter_map/flutter_map.dart';
import 'package:latlong2/latlong.dart';
import 'package:pettrack/widgets/pet_list_widgets.dart';
import 'package:pettrack/screens/image_search_screen.dart';
import 'package:cached_network_image/cached_network_image.dart';
import 'package:intl/intl.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        currentIndex: _selectedIndex,
        selectedItemColor: Color.fromARGB(255, 6, 228, 195),
        unselectedItemColor: const Color.fromARGB(255, 50, 49, 49),
        showUnselectedLabels: true,
        type: BottomNavigationBarType.fixed,
        onTap: _onItemTapped,
      ),
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              children: [
                FilterableLostFoundPetListView(status: 'lost', searchQuery: searchQuery),
                FilterableLostFoundPetListView(status: 'found', searchQuery: searchQuery),
              ],
            ),
          ),
        ],
...
// Updated Filterable Pet List View Widget for Lost/Found Pets
        ...
        final List<LostFoundPetModel> filteredPets = searchQuery.isEmpty
            ...
            : allPets.where((pet) {
                return pet.name.toLowerCase().contains(searchQuery) ||
                    ...
                    (pet.description?.toLowerCase().contains(searchQuery) ??
                        false) ||
                    (pet.petType?.toLowerCase().contains(searchQuery) ??
                        false);
              }).toList();
...
// Updated Pet Card Widget for Lost/Found Pets
...
// Updated Map Tab - Shows lost/found pets on map
class MapTab extends StatefulWidget {
  const MapTab({super.key});
...
class _MapTabState extends State<MapTab> {
  ...
  Future<void> _loadPetMarkers() async {
    try {
      final pets = await _lostFoundPetService.getAllLostFoundPetsOnce();
      setState(() {
        _markers = pets
            .where((pet) => pet.latitude != 0 && pet.longitude != 0)
            .map((pet) {
            ...
            child: GestureDetector(
              onTap: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) =>
                        LostFoundPetDetailScreen(petId: pet.id!),
                  ),
                );
              },
              child: Column(
